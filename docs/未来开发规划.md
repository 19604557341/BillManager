...
### 1. 账单新增接口
- `POST /api/bills`，新增 `BillCreateDTO`：
  - `billAmount`：`@NotNull` + `@DecimalMin("0.01")` + `@Digits(integer=13, fraction=2)`，与数据库 `DECIMAL(15,2)` 对齐。
  - `billType`：限定 `INCOME` / `EXPENSE`（建议引入枚举 + 自定义校验注解）。
  - `categoryId`：`@NotNull`，业务层需校验分类存在、处于启用状态、且分类类型与账单类型一致。
  - `billDate`：`@NotNull`；`remark`：`@Size(max=500)`。

### 2. 账单修改接口
- `PUT /api/bills/{billId}`，新增 `BillUpdateDTO`。
- 业务校验同新增；账单不存在时抛 404 业务异常。
...
| TD-3 | `BillService.getBillPage` 接口上的 `@Valid` 及 `@Min/@Max/@NotNull` 导入无效（校验应放在 Controller 层） | [`BillService.java`](../src/main/java/com/example/billmanager/service/BillService.java) | 移除接口层校验注解与无用导入 |
| TD-4 | `BillService.getBillPage` 的 Javadoc 仍描述旧签名（page/size 参数） | [`BillService.java`](../src/main/java/com/example/billmanager/service/BillService.java) | 更新注释为 `billQueryDTO` |
...
timeline
    ...
    已完成 : 项目搭建 : 分类管理 CRUD : 账单查询与分页
    短期 P0 : 账单新增 : 账单修改 : 账单删除 : 枚举治理
